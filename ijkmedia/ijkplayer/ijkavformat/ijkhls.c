...
static void free_playlist_list(FS_HLSContext *c)
{
    int i;
...
static void free_variant_list(FS_HLSContext *c)
{
    int i;
...
static void free_rendition_list(FS_HLSContext *c)
{
    int i;
                          ...
                          struct fs_playlist *pls, AVIOContext *in)
{
    ...
    while (!avio_feof(in)) {
        ...
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
            ...
            is_segment = 1;
            duration   = (int64_t)(atof(ptr) * AV_TIME_BASE);
                         ...
                         uint8_t *buf, int buf_size)
{
    ...
    if (seg->size >= 0)
        buf_size = (int)FFMIN((int64_t)buf_size, seg->size - pls->cur_seg_offset);
                      ...
                      ID3v2ExtraMetaAPIC **apic, ID3v2ExtraMeta **extra_meta)
{
    ...
    for (meta = *extra_meta; meta; meta = meta->next) {
        if (!strcmp(meta->tag, "PRIV")) {
            ...
            if (priv->datasize == 8 && !av_strncasecmp((const char *)priv->owner, id3_priv_owner_ts, 44)) {
            ...
            } else if (priv->datasize >= 8 && !av_strncasecmp((const char *)priv->owner, id3_priv_owner_audio_setup, 36)) {
                                      ...
                                      int64_t ts_b, struct fs_playlist *pls_b)
{
    ...
    return (int)av_compare_mod(scaled_ts_a, scaled_ts_b, 1LL << 33);
}
...
static int hls_probe(const AVProbeData *p)
{
    ...
    if (strncmp((const char *)p->buf, "#EXTM3U", 7))
        return 0;

    if (strstr((const char *)p->buf, "#EXT-X-STREAM-INF:")     ||
        strstr((const char *)p->buf, "#EXT-X-TARGETDURATION:") ||
        strstr((const char *)p->buf, "#EXT-X-MEDIA-SEQUENCE:")) {
...
#define OFFSET(x) offsetof(FS_HLSContext, x)
#define FLAGS AV_OPT_FLAG_DECODING_PARAM
static const AVOption fs_hls_options[] = {
...
static const AVClass fs_hls_class = {
    .class_name = "hls demuxer",
    .item_name  = av_default_item_name,
    .option     = fs_hls_options,
...
const FFInputFormat ijkff_ijkplaceholder1_demuxer = {
    ...
    .priv_data_size = sizeof(FS_HLSContext),
...
const AVInputFormat ijkff_ijkplaceholder1_demuxer = {
    .name           = "fs_hls",
    .long_name      = NULL_IF_CONFIG_SMALL("FSPlayer HLS"),
    .priv_class     = &fs_hls_class,
    .priv_data_size = sizeof(FS_HLSContext),
